...
const SchedulePreview = ({
  ...
  return (
    <div className="border border-gray-200 rounded-2xl p-4 bg-white">
      <h4 className="text-sm font-semibold text-gray-800 mb-3">Visualização do mês</h4>
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        {dates.map((date) => {
          const dayInfo = getDayInfo(date, schedule);
          return (
            <div key={date.toISOString()} className="rounded-2xl border border-gray-200 p-3 bg-gray-50">
              <div className="text-xs text-gray-500 mb-1">{dayLabel[weekdayFromIndex[date.getDay()]]}, {date.getDate()}</div>
              <div className={`rounded-xl px-2 py-1 text-xs font-medium ${dayInfo.color}`}>
                {dayInfo.status}
              </div>
            </div>
          );
